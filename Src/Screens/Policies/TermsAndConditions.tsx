// Src/Screens/Policies/TermsAndConditions.tsx
...
interface TermsLanguageData {
  title: string;
  sections: TermsSection[];
}

const TermsAndConditions = ({ navigation }: TermsAndConditionsProps) => {
  const [language, setLanguage] = useState<Language>('english'); // Default to English

  const { COLORS, SIZES, FONTS, ELEVATION, STYLES, moderateScale } = theme;

  const termsData: Record<Language, TermsLanguageData> = {
    english: {
      ...
      sections: [
        ...
        },
        {
          ...
          points: ['Cash', 'Credit Card', 'Debit Card', 'NEFT', 'RTGS', 'GPay'],
        },
        {
  ...
  const styles = StyleSheet.create({
    ...
    contactSection: {
      ...
      backgroundColor: COLORS.accentSoft,
      borderWidth: 1,
      borderColor: COLORS.borderAccent + '20',
      alignItems: 'center',
      padding: SIZES.space.lg,
    },
    contactTitle: {
  ...
  const LanguageToggle = () => (
    <View style={styles.languageToggleContainer}>
      <TouchableOpacity style={[styles.languageButton, language === 'english' && styles.activeLanguageButton]} onPress={() => setLanguage('english')}>
        <Text style={[styles.languageButtonText, language === 'english' && styles.activeLanguageButtonText]}>English</Text>
      </TouchableOpacity>
      <TouchableOpacity style={[styles.languageButton, language === 'tamil' && styles.activeLanguageButton]} onPress={() => setLanguage('tamil')}>
        <Text style={[styles.languageButtonText, language === 'tamil' && styles.activeLanguageButtonText]}>தமிழ்</Text>
      </TouchableOpacity>
    </View>
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.planHeader}>
          ...
          <Text style={styles.planSubtitle}>{language === 'english' ? 'Terms & Conditions' : 'விதிமுறைகள் மற்றும் நிபந்தனைகள்'}</Text>
        ...
        <View style={styles.disclaimerCard}>
          <Text style={styles.disclaimerTitle}>{language === 'english' ? 'Important Note' : 'முக்கிய குறிப்பு'}</Text>
        ...
        <View style={styles.contactSection}>
          <Text style={styles.contactTitle}>{language === 'english' ? 'For More Information' : 'மேலும் தகவலுக்கு'}</Text>
